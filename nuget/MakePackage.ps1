...
pushd ..
If (Test-Path LabelGenerator\bin\Release){
	del LabelGenerator\bin\Release\*.dll
}
...
Create-Nuspec("Kentor.LabelGenerator")
...
.\nuget\nuget.exe pack -IncludeReferencedProjects -build -outputdirectory nuget LabelGenerator\LabelGenerator.csproj
